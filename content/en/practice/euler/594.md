...
description: "For a polygon P, let t(P) be the number of ways in which P can be tiled using rhombi and squares with edge length 1."
...
$$M^{(u)}_{ij} = \binom{ (x_{j,u}-x_{i,u-1})+(y_{j,u}-y_{i,u-1}) }{ (x_{j,u}-x_{i,u-1})+(j-i) },$$

and

$$P^{(v)}_{ij} = \binom{ (x_{v,j}-x_{v-1,i})+(y_{v-1,i}-y_{v,j}) }{ (x_{v,j}-x_{v-1,i})+(j-i) }.$$
