...
$$A(n)=\frac1n\sum_{i=1}^n \operatorname{lcm}(n,i),
\qquad
S(n)=\sum_{k=1}^n A(k).$$
...
$$A(n)
= \frac1n \sum_{i=1}^n \frac{ni}{\gcd(n,i)}
...
=
\sum_{d\mid n}
\sum_{\substack{1\le m\le n/d\\ \gcd(m,n/d)=1}}
m.$$
...
=
\sum_{t\mid n}
\sum_{\substack{1\le m\le t\\ \gcd(m,t)=1}}
m.$$
...
$$\sum_{\substack{1\le m\le t\\ \gcd(m,t)=1}} m
=
\frac{t\varphi(t)}2.$$
...
$$f(1)=1,\qquad
f(t)=\frac{t\varphi(t)}2 \quad (t>1).$$
...
$$S(N)
=
\sum_{n\le N}\sum_{d\mid n} f(d).$$

Swap the order:

$$S(N)
=
\sum_{d\le N} f(d)\left\lfloor \frac{N}{d}\right\rfloor.$$
...
\frac12
\sum_{d=2}^N
d\varphi(d)\left\lfloor \frac{N}{d}\right\rfloor.$$
