...
description: "In the hexadecimal number system numbers are represented using different digits: The hexadecimal number when written in"
...
$$f(n,F)
=
\text{first}\times (16-|F|)^{n-1}$$

Now apply inclusion–exclusion:

$$\#(\text{contains }0,1,A)
=
\sum_{F\subseteq\{0,1,A\}}
(-1)^{|F|}
f(n,F)$$
