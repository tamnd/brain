...
description: "Each string of the ukulele is described by a starting pitch si. Moving to a higher fret increases the pitch by exactly one. If we only look at frets from l to r, then string i contributes every integer in the interval $$[si+l,; si+r]."
date: "2026-06-12T04:29:33+07:00"
...
[CF 1119D - Frets On Fire](https://codeforces.com/problemset/problem/1119/D)
...
Each string of the ukulele is described by a starting pitch `s_i`. Moving to a higher fret increases the pitch by exactly one. If we only look at frets from `l` to `r`, then string `i` contributes every integer in the interval
