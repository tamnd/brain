---
title: "CF 103092F - Finding Diamonds"
description: "The function $tau(x)$ in Section 7.2.1.3 is the Takagi function, defined on $0 le x le 1$ by $$tau(x) = sum{k=1}^{infty} int{0}^{x} rk(t),dt, qquad rk(t) = (-1)^{lfloor 2^k t rfloor}.$$ For each real $r$, define the level set $$L(r) = {x in [0,1] : tau(x) = r}."
...
[CF 103092F - Finding Diamonds](https://codeforces.com/problemset/problem/103092/F)
