...
description: "The problem asks whether string s can be formed from string t by deleting some characters from t without changing the order of the remaining characters. A subsequence does not require characters to be adjacent. The only requirement is that the relative ordering stays the same."
