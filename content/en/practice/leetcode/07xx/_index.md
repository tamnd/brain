---
title: "LeetCode 07xx"
description: "LeetCode problems 700–799"
tags: ["leetcode", "algorithms", "practice"]
weight: 7
draft: false
---

# LeetCode 07xx

| # | Problem | Difficulty | Time | Notes |
|---|---|---|---|---|
| 701 | [Insert into a Binary Search Tree](701.md) | 🟡 Medium | 1m 18s | This problem asks us to insert a new value into an existing Binary … |
| 702 | [Search in a Sorted Array of Unknown Size](702.md) | 🟡 Medium | 1m 21s | This problem asks us to search for a target value inside a sorted a… |
| 703 | [Kth Largest Element in a Stream](703.md) | 🟢 Easy | 27s | The problem asks us to design a data structure that continuously tr… |
| 704 | [Binary Search](704.md) | 🟢 Easy | 39s | This problem gives us a sorted array of integers named nums and a t… |
| 705 | [Design HashSet](705.md) | 🟢 Easy | 1m 39s | The problem asks us to design our own HashSet implementation withou… |
| 706 | [Design HashMap](706.md) | 🟢 Easy | 38s | The problem requires us to implement a HashMap from scratch without… |
| 707 | [Design Linked List](707.md) | 🟡 Medium | 1m 14s | This problem asks us to implement our own linked list data structur… |
| 708 | [Insert into a Sorted Circular Linked List](708.md) | 🟡 Medium | 47s | Let's go step by step and create a complete technical solution guid… |
| 709 | [To Lower Case](709.md) | 🟢 Easy | 55s | The problem asks us to convert every uppercase English letter in a … |
| 710 | [Random Pick with Blacklist](710.md) | 🔴 Hard | 1m 18s | The problem asks us to design a data structure that can repeatedly … |
| 711 | [Number of Distinct Islands II](711.md) | 🔴 Hard | 1m 36s | In this problem, we are given a binary matrix where each cell conta… |
| 712 | [Minimum ASCII Delete Sum for Two Strings](712.md) | 🟡 Medium | 1m 28s | The problem gives us two strings, s1 and s2, and asks us to make th… |
| 713 | [Subarray Product Less Than K](713.md) | 🟡 Medium | 1m 28s | The problem asks us to count how many contiguous subarrays of a giv… |
| 714 | [Best Time to Buy and Sell Stock with Transaction Fee](714.md) | 🟡 Medium | 1m 11s | This problem asks us to maximize profit from stock trading over a s… |
| 715 | [Range Module](715.md) | 🔴 Hard | 1m 21s | This problem asks us to design a data structure that dynamically tr… |
| 716 | [Max Stack](716.md) | 🔴 Hard | 37s | The problem asks us to design a stack data structure, MaxStack, whi… |
| 717 | [1-bit and 2-bit Characters](717.md) | 🟢 Easy | 1m 11s | In this problem, we are given a binary array called bits. The array… |
| 718 | [Maximum Length of Repeated Subarray](718.md) | 🟡 Medium | 1m 15s | The problem asks us to find the longest contiguous subarray that ap… |
| 719 | [Find K-th Smallest Pair Distance](719.md) | 🔴 Hard | 1m 6s | This problem asks us to find the k-th smallest distance among every… |
| 720 | [Longest Word in Dictionary](720.md) | 🟡 Medium | 1m 22s | This problem gives us an array of lowercase English words and asks … |
| 721 | [Accounts Merge](721.md) | 🟡 Medium | 1m 14s | This problem asks us to merge user accounts based on shared email a… |
| 722 | [Remove Comments](722.md) | 🟡 Medium | 43s | The problem asks us to remove comments from a C++ program represent… |
| 723 | [Candy Crush](723.md) | 🟡 Medium | 39s | This problem is essentially a simulation of the Candy Crush game, w… |
| 724 | [Find Pivot Index](724.md) | 🟢 Easy | 1m 4s | The problem asks us to find an index in the array such that the sum… |
| 725 | [Split Linked List in Parts](725.md) | 🟡 Medium | 1m | The problem gives us the head of a singly linked list and an intege… |
| 726 | [Number of Atoms](726.md) | 🔴 Hard | 39s | This problem asks us to parse a chemical formula represented as a s… |
| 727 | [Minimum Window Subsequence](727.md) | 🔴 Hard | 1m 31s | This problem asks us to find the smallest contiguous substring of s… |
| 728 | [Self Dividing Numbers](728.md) | 🟢 Easy | 1m 17s | The problem asks us to find all numbers within a given inclusive ra… |
| 729 | [My Calendar I](729.md) | 🟡 Medium | 1m 6s | The problem asks us to design a calendar system that supports booki… |
| 730 | [Count Different Palindromic Subsequences](730.md) | 🔴 Hard | 1m 23s | This problem asks us to count how many distinct non-empty palindrom… |
| 731 | [My Calendar II](731.md) | 🟡 Medium | 1m 14s | The problem asks us to design a calendar system that supports booki… |
| 732 | [My Calendar III](732.md) | 🔴 Hard | 1m 19s | This problem asks us to design a calendar system that supports addi… |
| 733 | [Flood Fill](733.md) | 🟢 Easy | 41s | The problem is asking us to simulate a flood fill operation on a 2D… |
| 734 | [Sentence Similarity](734.md) | 🟢 Easy | 1m 9s | The problem gives us two sentences, where each sentence is represen… |
| 735 | [Asteroid Collision](735.md) | 🟡 Medium | 1m 23s | The problem gives us a list of integers representing asteroids movi… |
| 736 | [Parse Lisp Expression](736.md) | 🔴 Hard | 1m 24s | The problem asks us to evaluate a Lisp-like expression represented … |
| 737 | [Sentence Similarity II](737.md) | 🟡 Medium | 1m 17s | This problem asks us to determine whether two sentences are conside… |
| 738 | [Monotone Increasing Digits](738.md) | 🟡 Medium | 1m 16s | The problem asks us to find the largest integer less than or equal … |
| 739 | [Daily Temperatures](739.md) | 🟡 Medium | 1m 18s | The problem gives an array called temperatures, where each element … |
| 740 | [Delete and Earn](740.md) | 🟡 Medium | 1m 10s | The problem gives us an integer array nums, and we want to maximize… |
| 741 | [Cherry Pickup](741.md) | 🔴 Hard | 1m 30s | This problem asks us to collect the maximum possible number of cher… |
| 742 | [Closest Leaf in a Binary Tree](742.md) | 🟡 Medium | 38s | The problem asks us to find the closest leaf node to a given target… |
| 743 | [Network Delay Time](743.md) | 🟡 Medium | 1m 7s | This problem models a directed weighted graph. Each node represents… |
| 744 | [Find Smallest Letter Greater Than Target](744.md) | 🟢 Easy | 1m 8s | The problem gives us a sorted array of lowercase English letters an… |
| 745 | [Prefix and Suffix Search](745.md) | 🔴 Hard | 1m 16s | The problem asks us to design a data structure that supports effici… |
| 746 | [Min Cost Climbing Stairs](746.md) | 🟢 Easy | 52s | This problem gives us an array named cost, where each element repre… |
| 747 | [Largest Number At Least Twice of Others](747.md) | 🟢 Easy | 1m 1s | The problem gives us an integer array nums in which the largest val… |
| 748 | [Shortest Completing Word](748.md) | 🟢 Easy | 1m 7s | The problem gives us a string called licensePlate and an array of c… |
| 749 | [Contain Virus](749.md) | 🔴 Hard | 1m 26s | This problem models the spread of a virus on a two-dimensional grid… |
| 750 | [Number Of Corner Rectangles](750.md) | 🟡 Medium | 57s | The problem asks us to count the number of corner rectangles in a b… |
| 751 | [IP to CIDR](751.md) | 🟡 Medium | 39s | This problem is asking us to take a starting IPv4 address and a num… |
| 752 | [Open the Lock](752.md) | 🟡 Medium | 30s | The problem presents a lock with four wheels, each wheel labeled fr… |
| 753 | [Cracking the Safe](753.md) | 🔴 Hard | 1m 11s | This problem asks us to generate the shortest possible string that … |
| 754 | [Reach a Number](754.md) | 🟡 Medium | 38s | The problem asks us to find the minimum number of moves required to… |
| 755 | [Pour Water](755.md) | 🟡 Medium | 1m 42s | This problem asks us to simulate how water droplets behave when pou… |
| 756 | [Pyramid Transition Matrix](756.md) | 🟡 Medium | 1m 31s | Here’s a full, detailed technical solution guide for LeetCode 756 -… |
| 757 | [Set Intersection Size At Least Two](757.md) | 🔴 Hard | 43s | The problem asks us to find the minimum size of a set of integers, … |
| 758 | [Bold Words in String](758.md) | 🟡 Medium | 1m 25s | The problem gives us two inputs: an array of strings called words, … |
| 759 | [Employee Free Time](759.md) | 🔴 Hard | 56s | The problem asks us to determine common free time intervals for a g… |
| 760 | [Find Anagram Mappings](760.md) | 🟢 Easy | 1m 24s | The problem asks us to build an index mapping from nums1 to nums2, … |
| 761 | [Special Binary String](761.md) | 🔴 Hard | 44s | The problem asks us to manipulate a special binary string to produc… |
| 762 | [Prime Number of Set Bits in Binary Representation](762.md) | 🟢 Easy | 47s | The problem requires counting the numbers within a given inclusive … |
| 763 | [Partition Labels](763.md) | 🟡 Medium | 43s | The problem is asking us to partition a string s into as many conti… |
| 764 | [Largest Plus Sign](764.md) | 🟡 Medium | 1m 12s | This problem asks us to find the largest axis-aligned plus sign mad… |
| 765 | [Couples Holding Hands](765.md) | 🔴 Hard | 1m 11s | This problem gives us a row of seats represented by the array row, … |
| 766 | [Toeplitz Matrix](766.md) | 🟢 Easy | 1m 4s | The problem asks us to determine whether a given matrix satisfies t… |
| 767 | [Reorganize String](767.md) | 🟡 Medium | 55s | The problem asks us to rearrange the characters of a given string s… |
| 768 | [Max Chunks To Make Sorted II](768.md) | 🔴 Hard | 1m 9s | The problem asks us to partition an array into the maximum possible… |
| 769 | [Max Chunks To Make Sorted](769.md) | 🟡 Medium | 41s | This problem is asking us to determine the maximum number of contig… |
| 770 | [Basic Calculator IV](770.md) | 🔴 Hard | 42s | The problem requires parsing and evaluating a mathematical expressi… |
| 771 | [Jewels and Stones](771.md) | 🟢 Easy | 1m 3s | In this problem, we are given two strings, jewels and stones. The s… |
| 772 | [Basic Calculator III](772.md) | 🔴 Hard | 1m 21s | This problem asks us to evaluate a mathematical expression represen… |
| 773 | [Sliding Puzzle](773.md) | 🔴 Hard | 45s | The Sliding Puzzle problem presents a 2 x 3 board containing five n… |
| 774 | [Minimize Max Distance to Gas Station](774.md) | 🔴 Hard | 1m 10s | The problem gives us a sorted array called stations, where each val… |
| 775 | [Global and Local Inversions](775.md) | 🟡 Medium | 40s | The problem presents an array nums of length n that is a permutatio… |
| 776 | [Split BST](776.md) | 🟡 Medium | 45s | The problem asks us to split a binary search tree (BST) into two se… |
| 777 | [Swap Adjacent in LR String](777.md) | 🟡 Medium | 1m 20s | The problem gives us two strings, start and result, both consisting… |
| 778 | [Swim in Rising Water](778.md) | 🔴 Hard | 1m 19s | In this problem, we are given an n x n grid where each cell contain… |
| 779 | [K-th Symbol in Grammar](779.md) | 🟡 Medium | 1m 2s | The problem defines a special binary grammar sequence that grows ro… |
| 780 | [Reaching Points](780.md) | 🔴 Hard | 40s | The problem asks whether it is possible to transform a starting poi… |
| 781 | [Rabbits in Forest](781.md) | 🟡 Medium | 56s | In this problem, every rabbit tells us how many other rabbits share… |
| 782 | [Transform to Chessboard](782.md) | 🔴 Hard | 1m 28s | This problem gives us an n x n binary matrix called board, where ev… |
| 783 | [Minimum Distance Between BST Nodes](783.md) | 🟢 Easy | 45s | The problem asks us to find the minimum difference between values o… |
| 784 | [Letter Case Permutation](784.md) | 🟡 Medium | 1m 6s | The problem asks us to generate every possible string that can be f… |
| 785 | [Is Graph Bipartite?](785.md) | 🟡 Medium | 39s | The problem asks whether a given undirected graph is bipartite. In … |
| 786 | [K-th Smallest Prime Fraction](786.md) | 🟡 Medium | 43s | The problem asks us to find the k-th smallest fraction that can be … |
| 787 | [Cheapest Flights Within K Stops](787.md) | 🟡 Medium | 56s | Here’s a complete, detailed technical solution guide for LeetCode 7… |
| 788 | [Rotated Digits](788.md) | 🟡 Medium | 1m 13s | The problem asks us to count the number of good integers within a g… |
| 789 | [Escape The Ghosts](789.md) | 🟡 Medium | 1m 40s | This problem takes place on an infinite two dimensional grid. You b… |
| 790 | [Domino and Tromino Tiling](790.md) | 🟡 Medium | 41s | The problem asks us to compute the number of ways to completely til… |
| 791 | [Custom Sort String](791.md) | 🟡 Medium | 1m 22s | The problem gives us two strings, order and s. The string order def… |
| 792 | [Number of Matching Subsequences](792.md) | 🟡 Medium | 57s | The problem asks us to count how many words in a given list are sub… |
| 793 | [Preimage Size of Factorial Zeroes Function](793.md) | 🔴 Hard | 53s | This problem asks us to determine how many non-negative integers x … |
| 795 | [Number of Subarrays with Bounded Maximum](795.md) | 🟡 Medium | 1m | The problem asks us to count how many contiguous, non-empty subarra… |
| 796 | [Rotate String](796.md) | 🟢 Easy | 59s | The problem asks us to determine whether one string can be transfor… |
| 797 | [All Paths From Source to Target](797.md) | 🟡 Medium | 58s | The problem gives us a directed acyclic graph, usually abbreviated … |
| 798 | [Smallest Rotation with Highest Score](798.md) | 🔴 Hard | 33s | The problem asks us to determine the optimal rotation of an array n… |
| 799 | [Champagne Tower](799.md) | 🟡 Medium | 33s | The problem asks us to simulate a process of pouring champagne into… |
