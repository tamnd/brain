...
description: "The problem asks us to design a cache that supports two operations: - get(key) returns the value associated with a key if it exists, otherwise -1 - put(key, value) inserts or updates a key-value pair Unlike a normal cache, eviction is not based only on recency."
