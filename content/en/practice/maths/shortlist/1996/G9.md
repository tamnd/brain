...
We first prove the result in the simplest case. Given a 2-gon $ABA$ and a point $O$, let $a, b, c, h$ denote $OA$, $OB$, $AB$, and the distance of $O$ from $AB$. Then $D = a + b$, $P = 2c$, and $H = 2h$, so we should show that

$$(a + b)^2 \geq 4h^2 + c^2.
\tag{1}$$

Indeed, let $l$ be the line through $O$ parallel to $AB$, and $D$ the point symmetric to $B$ with respect to $l$. Then $(a + b)^2 = (OA + OB)^2 = (OA + OD)^2 \geq AD^2 = c^2 + 4h^2$. Now we pass to the general case. Let $A_1A_2 \cdots A_n$ be the polygon $F$ and denote by $d_i, p_i,$ and $h_i$ respectively $OA_i$, $A_iA_{i+1}$, and the distance of $O$ from $A_iA_{i+1}$ (where $A_{n+1} = A_1$). By the case proved above, we have for each $i$,
