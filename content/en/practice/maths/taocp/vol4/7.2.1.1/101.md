...
**Exercise 101.** &#9654; [*M30*] [M30] (Unique factorization of strings into nonincreasing primes.)
a) Prove that if λ and λ′ are prime, then λλ′ is prime if λ < λ′.
b) Consequently every string α can be written in the form
α = λ1λ2 . . . λt,
λ1 ≥λ2 ≥· · · ≥λt,
where each λj is prime.
c) In fact, only one such factorization is possible. Hint: Show that λt must be the
lexicographically smallest nonempty suffix of α.
d) True or false: λ1 is the longest prime prefix of α.
