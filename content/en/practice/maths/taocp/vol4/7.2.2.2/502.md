...
**Exercise 502.** [*16*] *(Closed strings.)* Given binary strings $s_1, \ldots, s_m$ of length $n$, and threshold parameters $r_1, \ldots, r_m$, construct clauses that are satisfiable by $x = x_1 \ldots x_n$ if and only if $x$ differs from $s_i$ in at least $r_i$ bit positions, for $1 \le j \le m$.
