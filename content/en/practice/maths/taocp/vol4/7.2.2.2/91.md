...
**Exercise 91.** [*M21*] Formally speaking, Alice is said to "starve" if there is (i) an infinite sequence of transitions $X_0 \to X_1 \to \cdots$ starting from the state $X_0$, and (ii) an infinite sequence $\theta_0, \theta_1, \ldots$ of Boolean "bumps" that changes infinitely often, such that (iii) Alice is in a "maybe" or "critical" state only a finite number of times. Prove that this can happen if and only if there is a starvation cycle (47) as discussed in the text.
