...
**Exercise 516.** [*M9*] [M9] Prove or disprove the *strong exponential time hypothesis*: "If $\tau < 2$, there is an integer $k$ such that no randomized algorithm can solve every $k$SAT problem in fewer than $\tau^n$ steps, where $n$ is the number of variables."
