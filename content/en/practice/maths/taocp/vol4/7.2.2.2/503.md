...
**Exercise 503.** [*M20*] *(Covering strings.)* Given $s_j$ and $r_j$ as in exercise 502, show that every string of length $n$ is within $r_j$ bits of some $s_j$ if and only if the closest string problem has no solution with parameters $r'_j = n - 1 - r_j$.
