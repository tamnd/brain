...
**Exercise 28.** [*HM42*] [HM42] (D. H. Lehmer.) Show that the Hardy–Ramanujan–Rademacher coeffi-
cients Ak(n) defined in (34) have the following remarkable properties:
a) If k is odd, then A2k(km + 4n + (k2 −1)/8) = A2(m)Ak(n).
b) If p is prime, pe > 2, and k ⊥2p, then
Apek(k2m + p2en −(k2 + p2e −1)/24) = (−1)[pe=4]Ape(m)Ak(n).
In this formula k2 + p2e −1 is a multiple of 24 if p or k is divisible by 2 or 3;
otherwise division by 24 should be done modulo pek.
c) If p is prime, |Ape(n)| < 2[p>2]pe/2.
d) If p is prime, Ape(n) ̸= 0 if and only if 1 −24n is a quadratic residue modulo p
and either e = 1 or 24n mod p ̸= 1.
e) The probability that Ak(n) = 0, when k is divisible by exactly t primes ≥5 and
