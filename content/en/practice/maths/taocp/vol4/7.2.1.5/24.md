---
title: "TAOCP 7.2.1.5 Exercise 24"
description: "Let F_\\alpha(x_1,\\dots,x_m)=\\frac{1}{m!"
date: "2026-07-04T16:12:27+07:00"
...
**Exercise 24.** [*HM35*] [HM35] Continuing the previous exercise, because the Bell numbers satisfy the periodic law $\varpi_{n+p^{p-1} \cdot N} \equiv \varpi_n \pmod{p^r}$, if $p$ is an odd prime. [*Hint:* Show that
$$x^p \equiv g_{r}(x) + p^{r-1} g_{r-1}(x) + \cdots + g_1(x) \pmod{p^r}$$
where $g_r(x) = g_r(x, p^{r-1}g_{r-1}(x), \ldots, p \cdot g_1(x))$ and $g_s(x) = (x^p - x)^s$.]
