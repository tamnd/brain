---
title: "TAOCP 7.2.1.6 Exercise 121"
...
date: "2026-07-05T06:29:28+07:00"
...
**Exercise 121.** [*M34*] [M34] (F. Neuman, 1964.) The *derivative* of a graph $G$ is the graph $G^{(1)}$ obtained by removing all vertices of degree 1 and the edges touching them. Prove that, when $T$ is a free tree, its square $T^2$ contains a Hamiltonian path if and only if its derivative has no vertex of degree greater than 4 and the following two additional conditions hold:

i) All vertices of degree 3 or 4 in $T^{(1)}$ lie on a single path.

ii) Between any two vertices of degree 4 in $T^{(1)}$, there is at least one vertex that has degree 2 in $T$.
