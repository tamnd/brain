---
title: "TAOCP 7.2.1.6 Exercise 26"
...
date: "2026-07-05T08:08:17+07:00"
...
**Exercise 26.** [*M31*] [M31] *(The Kreweras lattice.)* Let $F$ and $F'$ be $n$-node forests with their nodes numbered 1 to $n$ in preorder. We write $F \prec F'$ ($F$ *coalesces* $F''$) if $j$ and $k$ are siblings in $F$ whenever they are siblings in $F'$, for $1 \le j < k \le n$. Figure 60 illustrates this partial ordering in the case $n = 4$; each forest is encoded by the sequence $c_1 \ldots c_n$ of (11) and (15), which specifies the depth of each node. (With this encoding, $j$ and $k$ are siblings if and only if $c_j = c_k \le c_{j+1}, \ldots, c_{k-1}$.)

![Figure 60: The Kreweras lattice of order 4. Each forest is represented by its sequence of node depths $c_1c_2c_3c_4$ in preorder. (See exercises 26–28.)](figure_60.png)

a) Let $\Pi$ be a partition of $\{1, \ldots, n\}$. Show that there exists a forest $F$, with nodes labeled $(1, \ldots, n)$ in preorder and with

$$j \equiv k \pmod{\Pi} \iff j \text{ is a sibling of } k \text{ in } F,$$

if and only if $\Pi$ satisfies the *noncrossing* property

$$i < j < k < l \text{ and } i \equiv k \pmod{\Pi} \text{ and } j \equiv l \pmod{\Pi} \implies i \equiv j \equiv k \equiv l \pmod{\Pi}.$$
...
c) Show that if $F'$ covers $F$, it has exactly one less leaf than $F$.

d) Show that if $F'$ covers $F$, when node $k$ has $c_k$ children for $1 \le k \le n$?

e) How many forests cover $F$, when node $k$ has $c_k$ children for $1 \le k \le n$?

f) Using the definition of duality in exercise 19, what is the dual of the forest $(2)$?

g) Prove that $F \prec F'$ holds if and only if $F^D \prec F^{\prime D}$. (Because of this property, dual elements have been placed symmetrically in the center of Fig. 60.)
h) Given any two $n$-node forests $F$ and $F'$, explain how to compute their greatest lower bound $F \wedge F'$; that is, $G \preccurlyeq F$ and $G \preccurlyeq F'$ if and only if $G \preccurlyeq F \wedge F'$.
i) Does this lattice satisfy a semimodular law analogous to exercise 7.1.5–12(f)?
