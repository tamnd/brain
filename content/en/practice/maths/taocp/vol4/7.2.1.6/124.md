---
title: "TAOCP 7.2.1.6 Exercise 124"
...
date: "2026-07-05T06:46:58+07:00"
...
**Exercise 124.** &#9654; [*40*] [40] Experiment with methods for drawing extended binary trees that are inspired by simple models from nature. For example, we can assign a value $v(x)$ to each node $x$, called its *Horton–Strahler number*, as follows: Each external (leaf) node has $v(x) = 0$; an internal node with children $(l, r)$ has $v(x) = \max(v(l), v(r)) + [v(l) = v(r)]$. The edge from internal node $x$ to its parent can be drawn as a rectangle with height $h(v(x))$ and width $w(v(x))$, and the edge rectangles with children $(l, r)$ can be offset by angles $\theta(v(l(x)), v(l(x)))$, $-\theta(v(r(x)), v(l(x)))$, for certain functions $h$, $w$, and $\theta$. The examples in Fig. 63 show typical results when we choose $w(k) = 3 + k$, $h(k) = 18k$, $\theta(k, k) = 30°$, $\theta(j, k) = ((k+1)/j) \times 20°$ for $0 \le k < j$, and $\theta(j, k) = ((k-j)/k) \times 30°$ for $0 \le j < k$; the roots appear at the bottom. Part (a) of Fig. 63 is the Fibonacci tree of order 11, which has 143 nodes; and part (d) is a random 100-node binary search tree. (The trees in parts (b), (c), and (d) clearly belong to different species.)
