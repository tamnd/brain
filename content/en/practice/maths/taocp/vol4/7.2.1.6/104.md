---
title: "TAOCP 7.2.1.6 Exercise 104"
...
date: "2026-07-05T04:42:21+07:00"
...
**Exercise 104.** &#9654; [*HM21*] If $G$ is a graph on $n$ vertices $\{V_1, \ldots, V_n\}$, with $e_{ij}$ edges between $V_i$ and $V_j$, let $C(G)$ be the matrix with entries $c_{ij} = -e_{ij} + \delta_{ij} d_i$, where $d_i = e_{i1} + \cdots + e_{in}$ is the degree of $V_i$. Let us say that the *aspects* of $G$ are the eigenvalues of $C(G)$, namely the roots $\alpha_0, \ldots, \alpha_{n-1}$ of the equation $\det(\alpha I - C(G)) = 0$. Since $C(G)$ is a symmetric matrix, its eigenvalues are real numbers, and we can assume that $\alpha_0 \le \alpha_1 \le \cdots \le \alpha_{n-1}$.

a) Prove that $\alpha_0 = 0$.

b) Prove that $G$ has exactly $e(G) = \alpha_1 \cdots \alpha_{n-1}/n$ spanning trees.

c) What are the aspects of the complete graph $K_n$?
