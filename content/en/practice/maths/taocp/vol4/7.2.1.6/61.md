---
title: "TAOCP 7.2.1.6 Exercise 61"
...
date: "2026-07-05T04:31:26+07:00"
...
**Exercise 61.** &#9654; [*M26*] (*Raney's Cycle Lemma.*) Let $b_1 b_2 \ldots b_N$ be a string of nonneg­ative integers such that $f = N - b_1 - b_2 - \cdots - b_N > 0$.

- a) Prove that exactly $f$ of the cyclic shifts $b_{k+1} \ldots b_N b_1 \ldots b_k$ for $1 \le j \le N$ satisfy the preorder degree sequence property in exercise 20.
- b) Design an efficient algorithm to determine all such $j$, given $b_1 b_2 \ldots b_N$.
- c) Explain how to generate a random forest that has $N = n_0 + \cdots + n_t$ nodes, with exactly $n_s$ nodes of degree $s$ for each $s$. (For example, we obtain random $n$-ary trees as a special case of this general procedure when $N = tn + 1$, $n_s = 0$ for $(t-1)n + 1$, $n_1 = \cdots = n_{t-1} = 0$, and $n_t = n$.)
