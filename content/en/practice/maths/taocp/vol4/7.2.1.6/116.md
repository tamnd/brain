---
title: "TAOCP 7.2.1.6 Exercise 116"
...
date: "2026-07-04T23:55:20+07:00"
...
**Exercise 116.** &#9654; [*28*] The nodes of a forest $F$ are labeled $1$ to $n$ in prepostorder; say that node $k$ is *lucky* if it is adjacent to node $k+1$ in $F$, *unlucky* if it is three steps away, and *ordinary* otherwise, for $1 \le k \le n$; in this definition, node $n+1$ is an imaginary super-root considered to be the parent of each root.

a) Prove that lucky nodes occur only on even-numbered levels; unlucky nodes occur only on odd-numbered levels.

b) Show that the number of lucky nodes is exactly one greater than the number of unlucky nodes, unless $n = 0$.
