...
Similarly,
$$BA=I,$$
since the same computation applies. Therefore $B=A^{-1}$. Consequently the inverse of a combinatorial matrix is again a combinatorial matrix, with entries
$$\boxed{ b_{ij}= \frac{-y+\delta_{ij}(x+ny)}{x(x+ny)} }.
