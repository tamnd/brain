...
description: "Let `T` be a pointer to a binary tree of $n$ nodes with the conventional representation `(2)`, that is, each node `P` has two link fields `LLINK(P)` and `RLINK(P)` pointing either to the left or right..."
