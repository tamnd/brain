...
description: "For the array `A[i,j,k]` with $0 \\le k \\le j \\le i \\le n$, storing the elements in lexicographic order of the indices means that `k` varies fastest, then `j`, then `i`."
