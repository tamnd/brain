...
description: "A denoising autoencoder learns to reconstruct a clean input from a corrupted version of that input. Instead of copying $x$ to $\\hat{x}$, the model receives a noisy input $\\tilde{x}$ and must recover the original $x$."
